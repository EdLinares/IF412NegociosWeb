  ...
  if(isset($_GET["action"])){
    switch($_GET["action"]){
      ...
      case "del":
        ...
        if(isset($_POST["btnInsert"])){
          if(isset($_POST["prdid"])){
            $strupdelete = "DELETE  FROM `productos` WHERE `prdid` = %d;";
            $strupdelete = sprintf($strupdelete,
                                intval($_POST["prdid"])
                                );
            updateRegistro($strupdelete);
            redirectWithMessage('Producto borrado!', 'ej9.php?action=del&prdid='.$_POST["prdid"]);
          }
        }
